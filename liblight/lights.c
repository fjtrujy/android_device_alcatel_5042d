...
char const*const WHITE_LED_FILE = "/sys/class/leds/led_G/brightness";

char const*const BUTTON_FILE = "/sys/class/leds/button-backlight/brightness";

char const*const WHITE_BLINK_FILE = "/sys/class/leds/led_G/blink";
...
static void set_speaker_light_locked(struct light_device_t *dev,
  ...
  if ((colorRGB >> 8) & 0xFF) color = LED_WHITE;
  if ((colorRGB >> 16) & 0xFF) color = LED_WHITE;
  if (((colorRGB >> 8) & 0xFF) > ((colorRGB >> 16) & 0xFF)) color = LED_WHITE;
    ...
    case LIGHT_FLASH_TIMED:
      switch (color) {
        case LED_WHITE:
          ...
          write_int(WHITE_BLINK_FILE, blinkMode);
          break;
        case LED_BLANK:
...
static void set_speaker_light_locked_dual(struct light_device_t *dev,
  ...
  if ((bcolorRGB >> 8) & 0xFF) bcolor = LED_WHITE;
  if ((bcolorRGB >> 16) & 0xFF) bcolor = LED_WHITE;
